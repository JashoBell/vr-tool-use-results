---
title: "VR - Trial-level Data - Reaching"
author: "Joshua Bell"
format:
  html:
    toc: true
...
ppt_m_fulltrials <- fread(here::here("data", "study_one", "ppt_m_fulltrials.csv"))
...
color_scale = c("steelblue", "forestgreen", "gold")
names(color_scale) = c("begin", "out", "lift")

ppt_m <- fread(here::here("data", "study_one", "ppt_manual.csv"))
ppt_m[,
  time := factor(time, levels = c("pre", "post"))
  ]
...
## PPT
```{r plotsbyparticipant, cache = FALSE}
...
#### Pre
```{r participant-1}
print(plots[["d1-001"]]["pre"])
```
#### Post
```{r participant-1-post}
print(plots[["d1-001"]]["post"])
```

### Participant 2
#### Pre
```{r participant-2}
print(plots[["d1-002"]]["pre"])
```
#### Post
```{r participant-2-post}
print(plots[["d1-002"]]["post"])
```
...
#### Pre
```{r participant-3}
print(plots[["d1-003"]]["pre"])
```

#### Post
Trials 8 and 9 of the post-test failed to save the PPT data.
```{r participant-3-post}
print(plots[["d1-003"]]["post"])
```

### Participant 4
#### Pre
```{r participant-4}
print(plots[["d1-004"]]["pre"])
```
#### Post
```{r participant-4-post}
print(plots[["d1-004"]]["post"])
```

### Participant 5
#### Pre
```{r participant-5}
print(plots[["d1-005"]]["pre"])
```
#### Post
```{r participant-5-post}
print(plots[["d1-005"]]["post"])
```
