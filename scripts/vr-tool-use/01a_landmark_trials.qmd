---
title: "VR - Trial-level Data - Landmark"
author: "Joshua Bell"
format:
  html:
    toc: true
...
landmark <- read_csv(here::here("data", "study_one", "landmark_full.csv")) %>%
...
landmark_fix <- read_csv(here::here("data", "study_one", "landmark_full.csv")) %>%
  select(!experiment:participant) %>%
  mutate(
...
landmark_long <- reshape2::melt(landmark,
...
landmark_long_fix <- reshape2::melt(landmark_fix,
  id.vars = c("ppid", "block", "time", "trial", "target"),
  measure.vars = colnames(select(landmark, ends_with(c("pos_z")))),
  variable.name = "armlocation",
  value.name = "position") %>% as.data.table()
...
for(p in unique(landmark$ppid))
  {
      ...
      plot <- ggplot(
        ...
        geom_text(aes(
          y = 1.85,
          x = mean(e[[p]][target == "Wrist"]$position) + .2,
        ...
        # Line labels ----------------------------------------
        geom_text(aes(
          x = mean(e[[p]][target == "Forearm"]$position) - .2,
          y = 1.15,
      ...
      if(p == "d1-015")
      {
        ggsave(here::here("scripts", "for_osf", "tooluse_in_vr", "landmark_trials_plot.svg"), dpi = 420, 
        plot = plot, bg = "transparent")
        saveRDS(plot, file = here::here("scripts", "for_osf", "tooluse_in_vr", "landmark_trials_plot.rds"))
      }
  }
...
pre_ratio_ <- unlist(pre_ratio) %>% reshape2::melt(value.name = "perceived_length") %>% mutate(time = "pre")
post_ratio_ <- unlist(post_ratio) %>% reshape2::melt(value.name = "perceived_length") %>% mutate(time = "post")
...
```{r}
armlength_df <- armlength_df %>% as.data.table()
armlength_df[order(veridical)]
change_df[order(veridical_change)]

change_df[, abs_change := perceived_change + veridical_change]
skimr::skim(change_df)
```

Participant 4's trackers were recorded as >4cm closer together in the post-test relative to the pre-test, despite being placed in the same position. It is unclear whether this is due to universe drift or researcher oversight. Participant 4's data was removed from the analysis.
...
Participant 6 had a high response variability of nearly half of their arm length. This participant was also left-handed. Participant 6's data was therefore removed from analysis.
