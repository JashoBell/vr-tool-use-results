...
rm(ppt_m_files, ppt_trials_nested, ppt_trials_unnested)
gc()
...
# Create buffer at beginning and end of each trial to reduce filter edge effects
...
rm(ppt_prefilter, buffer)
gc()


# Apply Butterworth filter to position values using data.table
ppt_position_filtered <- ppt_prefilter_buffered[!(ppid %like% "pilot"), ][
  order(ppid, block_num, trial_num_in_block, time, tracked, time_ms)
][,
  `:=`(
    pos_x_10hz = filtfilt(b_10hz, pos_x),
    pos_y_10hz = filtfilt(b_10hz, pos_y),
    pos_z_10hz = filtfilt(b_10hz, pos_z)
  ),
  by = .(ppid, block_num, trial_num_in_block, time, tracked)
]
...
rm(ppt_position_filtered, ppt_prefilter_buffered)
gc()
```
