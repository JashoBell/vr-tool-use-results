---
title: "Avatar VR - Supplementary Analyses - Landmark Task"
author: "Joshua Bell"
date: "10/14/2022"
format:
  html:
...
## Fatigue Effects on Landmark Task
...
stat_dotsinterval(position = position_dodge(.5), point_interval = "mean_qi") +
facet_grid(~target) +
theme_tidybayes() +
scale_fill_manual(breaks = c("No", "Yes"),
  values = c("#001015", "#09A1D7"),
  labels = c("No", "Yes")) +
labs(y = "Error (cm)", x = "Time", fill = "Fatigue", title = "Body Landmark Task Response Distributions") +
theme(axis.text.y = element_text(hjust = 0),
      title = element_text(size = 16),
      text = element_text(size = 14, color = "black"),
      strip.background = element_blank(),
...
lm_perceived <- update(
  ...
  refresh = 0,
  seed = 1337,
  file = here::here("scripts", "vr-tool-use-avatar", "models", "supplementary", "landmark_perceived.rds")
  )
