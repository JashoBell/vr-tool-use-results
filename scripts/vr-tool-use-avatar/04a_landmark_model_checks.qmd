---
title: "Avatar VR - Posteriors - Landmark"
author: "Joshua Bell"
format:
  html:
    toc: true
...
pkgs <- c("dplyr", "tidyr", "magrittr", "readr", "stringr", "data.table", "here", "ggplot2", "ggthemes", "flextable", "officer", "equatags", "tidybayes", "brms", "rstantools", "datawizard", "priorsense")
for(i in pkgs){
...
#Empirical data
landmark <- read_csv(here::here("data", "study_three", "landmark.csv")) %>% as.data.table()
...
#Model data
landmark_model <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "models", "landmark_error_time_target.rds"))
landmark_lapsefix_model <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "models","landmark_lapsefix_error_time_target.rds"))
```
...
ggplot(aes(y = .epred,
  ...
  scale_color_manual(breaks = c("Pre", "Post"),
    values = c("#001015", "#09A1D7"),
    labels = c("Pre", "Post")) +
  labs(y = "Error (cm)", x = "Time", fill = "Time", color = "Time") +
...
saveRDS(landmark_posterior_expected_value_plot, here::here("scripts", "for_osf", "tooluse_avatar", "figures", "landmark_posterior_parameter_plot.rds"))
```
...
saveRDS(landmark_posterior_predictive_plot, here::here("scripts", "for_osf", "tooluse_avatar", "figures", "landmark-posterior-prediction-plot.rds"))
```
## Model Summary

```{r}
model_params <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "tables", "landmark_model_params.rds"))
...
fwrite(draws_change, here::here("scripts", "for_osf", "tooluse_avatar", "plot_data", "landmark_draws_change.csv"))
...
model_table <- model_params_unstd %>%
  ...
  hline(i = 3, 
        border = officer::fp_border(style = "solid", 
                                    width = 1.5), 
        part = "body") %>%
  style(i = 2:3,j = 2, part = "body", pr_t = fp_text(font.size = 9)) %>%
  style(i = 1,j = 2, part = "body", pr_t = fp_text(font.size = 12)) %>%
...
saveRDS(model_table, here::here("scripts", "for_osf", "tooluse_avatar", "tables", "landmark_table.rds"))
```

```{r}
# lapsefix_model_params <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "tables", "landmark_lapsefix_model_params.rds"))
...
#            value = as_paragraph(
#              as_qmd(
#                "Parameter estimates (Mode, 95% HDI) based on 50,000 draws from the posterior distribution, unstandardized and calculated as (Post - Pre). Values correspond to error in cm, with negative values indicating an underestimate of the landmark on the z (forward) axis. Reported Bayes Factors represent multiplicative evidence in favor of one hypothesis over another. BF > 1 indicate support for the expected effect of tool-use, while fractional values indicate that evidence favors the complimentary null."
#                )
#              ), 
#            ref_symbols = c("a", "b"),
#            part = "header"
...
# # Save the table to an RDS to be used in the manuscript
# saveRDS(lapsefix_model_table, here::here("scripts", "for_osf", "tooluse_avatar", "tables", "lapsefix_landmark_table.rds"))
```
...
```{r}
model <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "models", "landmark_error_time_target.rds"))
model_lapsefix <- readRDS(here::here("scripts", "for_osf", "tooluse_avatar", "models", "landmark_lapsefix_error_time_target.rds"))
